...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() => ref.read(orderProvider.notifier).getOrders());
  }
              ...
              if (orderState.orders.isNotEmpty)
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      _StatItem(label: "Total Orders", value: orderState.orders.length.toString(), icon: Icons.receipt_long, color: AppColors.primary),
                      Container(width: 1, height: 40, color: AppColors.border),
                      _StatItem(
                        label: "Active",
                        value: orderState.orders.where((o) => o["status"] == "active").length.toString(),
                      ...
                      _StatItem(
                        label: "Completed",
                        value: orderState.orders.where((o) => o["status"] == "completed").length.toString(),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    _ProfileMenuItem(icon: Icons.logout, label: "Logout", isRed: true, isLast: true, onTap: _logout),
                  ],
                ),
              ),
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Text(value, style: TextStyle(fontSize: R.sp(context, 18), fontWeight: FontWeight.w800, color: color)),
        Text(label, style: TextStyle(fontSize: R.sp(context, 11), color: AppColors.textMuted)),
      ],
    );
  }
}
...
class _ProfileMenuItem extends StatelessWidget {
  ...
  const _ProfileMenuItem({required this.icon, required this.label, this.isActive = false, this.isRed = false, this.isLast = false, required this.onTap});
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    label,
                    style: TextStyle(fontSize: R.sp(context, 14), fontWeight: isActive ? FontWeight.w600 : FontWeight.w400, color: color),
                  ),
                ),
                if (!isRed) Icon(Icons.chevron_right, color: Colors.grey.shade400, size: 20),
              ],
            ),
          ),
